...
function Onboarding({ th, Ic, onDone }) {
  const slides = [
  { img: window.assetURL('assets/img/brooklyn-rust.jpg'), title: 'Find your next icon', body: 'Tell me a colour, a style or a budget — I’ll bring the pieces straight to you.' },
  { img: window.assetURL('assets/img/tabby-brown.jpg'), title: 'The perfect gift', body: 'Share who it’s for and the occasion, and I’ll curate a considered few.' },
  { img: window.assetURL('assets/img/icon-lana.jpg'), title: 'Here to help', body: 'Tabby & Tamara, delivery, returns, store hours — just ask, anytime.' }];
    ...
    <div className="ca-msg-in" style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingTop: 4 }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
        ...
        <div style={{ background: th.botBg, borderRadius: th.bubbleR, padding: '12px 15px', fontSize: 14.5, lineHeight: 1.5, color: th.botText }}>
          Hello — I’m your Coach shopping assistant. How can I help you today?
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginLeft: 37 }}>
        {actions.map((a) =>
        <button key={a.key} onClick={a.run} style={{ textAlign: 'left', background: th.cardBg, border: th.cardBorder, borderRadius: th.cardR,
          padding: '14px 13px', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 9, minHeight: 96, fontFamily: 'var(--sans)' }}>
            <span style={{ color: th.accent }}><a.icon size={21} /></span>
            <span>
  ...
  return (
    <div style={{ background: th.cardBg, border: `1px solid ${th.accent}`, borderRadius: th.cardR, padding: '16px 16px 15px', maxWidth: 320 }}>
      ...
      <p style={{ fontSize: 13, lineHeight: 1.6, color: th.sub, margin: '0 0 6px' }}>
        We’re online between <strong style={{ color: th.text }}>10am and 10pm UAE time</strong>, every day. Drop us an email with your query and we’ll get back to you as soon as we can.</p>
