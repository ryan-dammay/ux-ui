...
function Spec({ name, device='both', wide=false, span2=false, ground='checker', states, parts, note, behaviour }) {
  return (
      ...
      {(parts || note || behaviour) &&
        <div className="meta">
          ...
          {behaviour && behaviour.map((b,i)=>(
            <div className="behav" key={i}>
              <h6><span className="ico">{b.icon}</span>{b.title}</h6>
              <ul className={'flow'+(b.variant?' '+b.variant:'')}>
...
const MENU_ITEMS = ['Find your next bag','Find the perfect gift','Payments & BNPL','Delivery','Returns','Store locations','Speak to Customer Care','Start over'];
function MenuList() {
  return (
...
const WELCOME_ACTIONS = [
  { key:'find', label:'Find your next bag', sub:'By colour, style or budget', icon:Ic.Search, run:noop, wide:true },
  { key:'gift', label:'Find the perfect gift', sub:'For someone special', icon:Ic.Gift, run:noop },
  ...
  { key:'care', label:'Speak to Customer Care', sub:'Talk to our team', icon:Ic.User, run:noop },
];
    ...
    <div id="root-inner">
      ...
      <Section n="01" title="Entry &amp; chat shell" desc="The container chrome. Panel differs by breakpoint; header, input and menu are shared.">
        <Spec name="Launcher bubble" device="both" ground="checker"
          parts={['button','shopping-cart icon','pulse ring']}
          note="Tap-only entry, bottom-right. Uses a <b>cart mark with a spark</b> (reads as a shopping assistant, not a chatbot). <b>Idle</b> shows a soft pulsing ring; suppressed while open."
          states={[
      ...
      <Section n="02" title="Onboarding &amp; welcome" desc="First-run intro (shown once) and the welcome menu.">
        <Spec name="Onboarding intro" device="both" ground="checker" span2={true}
          ...
          note="Swipeable intro (3 slides: Find your next bag · Find the perfect gift · Here to help) — <b>shown every time the assistant opens</b>. Live — drag or tap Next."
        ...
        <Spec name="Welcome" device="both" ground="panel" span2={true}
          parts={['avatar','greeting bubble','hero card (Find your next bag)','action card ×4','card icon','card title','card subtitle']}
          note="Greeting + quick-start actions. <b>Find your next bag</b> is a full-width hero; the rest pair into two rows: Find the perfect gift / Payments &amp; BNPL, then Delivery &amp; returns / <b>Speak to Customer Care</b> (last). Cards use a 1.5px stroke, 4px radius."
      ...
      <Section n="03" title="Message types" desc="Everything that can appear in the thread. Identical across breakpoints.">
        ...
        <Spec name="Agent message" device="both" ground="panel"
          ...
          states={[{node:<div style={{background:th.botBg,color:th.botText,borderRadius:th.bubbleR,padding:'11px 14px',fontSize:13.5,lineHeight:1.55,borderLeft:`2px solid ${th.accent}`,maxWidth:290}}>
            ...
            Hello, I’m Layla from Coach Customer Care. How can I help you today?</div>}]} />
      ...
      {/* PAYMENTS */}
      <Section n="06" title="Payments &amp; BNPL" desc="Informational only: how Tabby &amp; Tamara work, payment methods, then an option to speak to Customer Care. No troubleshooting.">
      ...
      {/* SUPPORT & HANDOVER */}
      <Section n="07" title="Support &amp; handover" desc="Store info and the handover to Customer Care (Layla) — via the dedicated “Speak to Customer Care” action, an upset message, or “Talk to a stylist”. Two states, gated by hours.">
        ...
        <Spec name="Agent card" device="both" ground="panel" span2={true}
          parts={['avatar (L monogram)','name','status dot','status text']}
          note="A <b>status surface</b>, not a control — it has no buttons. It marks the moment the conversation passes from the assistant to a human (Layla). The card stays in the thread; the chat then continues with Layla’s messages and quick replies."
          behaviour={[
            { title:'When it is shown', icon:<Ic.Help size={13} />, items:[
              'User taps the dedicated <b>“Speak to Customer Care”</b> action (welcome card, menu, or Payments) — the assistant sends the connect line, then this card.',
              'Free-text shows <b>frustration or asks for a human</b> (e.g. “I’m angry”, “speak to customer care”, “refund”) — routed straight to Care, with an empathetic line if upset.',
              'User taps <b>“Talk to a stylist”</b> (discovery / gifting) or FAQ <b>“Something else”</b>.',
              '<b>Only within Customer Care hours (10am–10pm UAE).</b> Outside hours the <b>Offline card</b> is shown instead — never the agent card.',
            ]},
            { title:'When it is NOT shown', icon:<Ic.Close size={13} />, variant:'no', items:[
              'Outside Customer Care hours → <b>Offline card</b> (save transcript / email) instead.',
              'Pure browsing, discovery, gifting, payment explainers, or answered FAQs — no human needed.',
              'Informational answers (how Tabby/Tamara works, payment methods) resolve in-thread.',
            ]},
            { title:'Interaction sequence', icon:<Ic.Send size={13} />, variant:'steps', items:[
              'Card appears in state <b>Connecting</b> — grey status dot, “Connecting you to a person…”. Typing indicator may precede it.',
              'After ~1.1–1.2s it transitions to <b>Connected</b> — green dot, “Connected — typically replies in ~2 min”. Same card, animated in place.',
              'Layla’s intro message follows as an <b>agent message</b> bubble (“How can I help you today?”).',
              'Conversation continues via Layla + a quick reply (<b>Thanks, Layla</b>). The card persists above as a marker.',
            ]},
          ]}
          states={[
