...
3. **Payments & BNPL** — how Tabby / Tamara work, payment methods (informational only)
4. **FAQs** — delivery, returns, store locations
5. **Speak to Customer Care** — a dedicated, one-tap handover to a human (Layla)
...
- **Out of scope (hand off or link out):** completing payment/checkout, order tracking,
  troubleshooting failed payments, order-specific account changes, anything requiring
...
- **Handover trigger:** the assistant connects to **Layla · Customer Care** when the user
  taps the dedicated **“Speak to Customer Care”** action, types something **upset or asks
  for a human** (e.g. “I’m angry”, “speak to someone”, “refund”), or taps **“Talk to a
  stylist”** / FAQ **“Something else”**. Within hours → live agent; outside hours →
