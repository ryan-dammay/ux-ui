...
- **Speakers:** Customer · Assistant (the bot) · Layla (Customer Care, after handover) · System.
...
## Flow C — Payments & BNPL  *(informational only)*

Entry menu: *How Tabby works* · *How Tamara works* · *Payment methods* ·
**Speak to Customer Care**.

The assistant **explains**, it does not troubleshoot. There is **no** payment-failure triage
and **no** voucher/promo flow — anything that needs investigating goes straight to a human.

### BNPL explainers
Tabby (split into 4 interest-free payments, 25% today) and Tamara (Pay in 4 **or** Pay in
30 days). Each shows a 4-instalment breakdown computed from a sample basket. Always
described as **interest-free**.

### Payment methods
Lists accepted methods (Visa, Mastercard, Amex, Apple Pay, Cash on Delivery) and notes
Tabby/Tamara at checkout.

### If the customer needs more
Every payments screen offers **Speak to Customer Care** — one tap to the live handover
(see Flow E). No bot-led “which card are you using?” interrogation.
...
## Flow E — Speak to Customer Care (dedicated handover)

A **first-class entry**, not just a fallback. The customer can reach a human directly via:

- the **“Speak to Customer Care”** welcome card (last in the grid) or ≡ menu item,
- the **Speak to Customer Care** option on any Payments screen,
- typing something **upset or asking for a human** (see Free-text routing).

**Sequence:**
1. On the explicit action the assistant sends the connect line —
   *“Of course! Let me connect you with our Customer Care team right away. 🙋”*
   (the one intentional emoji). If the customer was **upset**, it leads instead with an
   empathetic line — *“I'm sorry to hear that. Let me connect you with our Customer Care
   team right away.”*
2. The **agent card** animates *Connecting… → Connected*.
3. **Layla** introduces herself: *“Hello, I'm Layla from Coach Customer Care. How can I
   help you today?”* → quick reply *Thanks, Layla*.

---

## Handover points

**Handover points** (marked `→ HANDOVER` in the exports) occur when:
- the customer taps **Speak to Customer Care** (welcome card, menu, or Payments),
- free text is **upset / asks for a human** (angry, complaint, refund, “speak to someone”),
- the customer taps **Talk to a stylist** (discovery / gifting) or FAQ **Something else**.
...
1. **Within hours → live agent.** Agent card animates *Connecting… → Connected*, then
   **Layla** introduces herself.
2. **Outside hours → offline notice.** “Our Customer Care team is currently offline… we're
   available 10am–10pm… email us.” Actions: **Close chat** · **Save transcript**.
...
Typed input is matched to a flow by keyword before falling back to the menu. **Order
matters** — the upset / human check runs first:
- **upset or asks for a human** (angry, upset, frustrated, terrible, complaint, refund,
  “speak/talk to someone”, “customer care”, “human”, “agent”, “manager”) →
  **Speak to Customer Care** (empathetic lead-in if upset)
- gift / present / birthday / anniversary → **Gifting**
- tabby / tamara / pay / payment / bnpl → **Payments** (informational)
- deliver / ship / return / exchange / store → **FAQ**
...
- [ ] Informational answers offer a path to Customer Care; upset input routes there first.
